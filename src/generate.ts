...
const githubUserToAuthor = (
  user: z.infer<typeof GitHubUser>,
): APIEmbedAuthor => ({
  name: user.login,
  icon_url:
    user.avatar_url ?? `https://avatars.githubusercontent.com/u/${user.id}?v=4`,
...
const getReactionsString = (
  reactions: z.infer<typeof GitHubReactions>,
): string | null => {
...
export const generateGithubPayload = async (request: IRequest) => {
  ...
  if (profileUsername && profileUsername.length > 128) {
    return json(
      { code: "ProxyBadUsername", message: "Username is too long." },
      { status: 400 },
    );
  }
  if (profileAvatarUrl) {
  ...
  if (!eventType_) {
    return json(
      { code: "ProxyNoEventType", message: "No event type provided." },
      { status: 400 },
    );
  }
  ...
  if (!eventTypeParse.success) {
    return json(
      { code: "ProxyBadEventType", message: "Unsupported event type." },
      { status: 400 },
    );
  }
  const eventType = eventTypeParse.data;
  const payloadValidator = GitHubEventTypeToPayload[eventType];
  if (payloadValidator) {
    const d = await payloadValidator.parseAsync({
      type: eventType,
    ...
    switch (d.type) {
      case "commit_comment":
        ...
        embed.url = `${getRepoUrl(d.pl.repository)}/commit/${
          d.pl.comment.commit_id
        }#commitcomment-${d.pl.comment.id}`;
        embed.description = d.pl.comment.body.slice(0, 2048);

        immContent = d.pl.comment.body;
        immEmbed.title = `Replying to commit ${shortCodeCommit(
          d.pl.comment.commit_id ?? "",
        )}`;
      ...
      case "create":
        embed.title = `${
          d.pl.ref_type === "branch" ? "Branch" : "Tag"
        } created: ${d.pl.ref}`;
      ...
      case "pull_request_review_comment": {
        ...
        immContent = d.pl.comment.body;
        immEmbed.title = `Replying to ${
          d.type === "issue_comment"
            ? `issue #${d.pl.issue.number}`
            : `pull request #${d.pl.pull_request.number}`
        }`;
        immEmbed.url = d.pl.comment.html_url;
      ...
      case "issues": {
        ...
        switch (d.pl.action) {
          case "opened":
            embed.color = green;
            embed.description = d.pl.issue.body
              ? d.pl.issue.body.slice(0, 2048)
              : undefined;
            break;
          case "closed":
            embed.color = red;
      ...
      case "ping":
        embed.title = "Ping";
        embed.description = `You have added this webhook correctly! Thanks for using [guilded-webhook-proxy](${
      ...
      //   break;
      case "pull_request":
        embed.title = `Pull request #${d.pl.number} ${d.pl.action.replace(
          /_/g,
          " ",
        )} - ${d.pl.pull_request.title}`;
        embed.url = d.pl.pull_request.html_url;

        switch (d.pl.action) {
          case "opened": {
            embed.color = green;
            embed.description = d.pl.pull_request.body
              ? d.pl.pull_request.body.slice(0, 2048)
              : undefined;
            if (d.pl.pull_request.user) {
              embed.author = githubUserToAuthor(d.pl.pull_request.user);
            }
            embed.fields = [];
            const assignees = d.pl.pull_request.assignees
              .filter(Boolean)
              .map((u) => u as NonNullable<typeof u>);
      ...
      case "pull_request_review":
        ...
        if (d.pl.review.body) {
          embed.description += " ";
          embed.description += d.pl.review.body.slice(
            0,
            2048 - embed.description.length,
          );
        }
      ...
      case "push":
        ...
        embed.title = `[${d.pl.ref.replace(
          /^refs?\/(tags|heads)\//,
          "",
        )}] ${d.pl.commits.length.toLocaleString()} new commit${
          d.pl.commits.length === 1 ? "" : "s"
        }`;
        embed.url = d.pl.compare;
        embed.description = "";
        for (const commit of d.pl.commits) {
          const line = `[${commit.id.slice(0, 8)}](${
            commit.url
      ...
      case "release":
        ...
        if (
          ...
          [
            ...
            "published",
            "deleted",
          ].includes(d.pl.action)
        ) {
          embed.description = d.pl.release.body.slice(0, 2048);
        }
        embed.title = `${d.pl.release.draft ? "Draft" : "Release"} ${
          d.pl.action
        } - ${d.pl.release.name ?? d.pl.release.tag_name}`;
      ...
      case "repository":
        ...
        if (
          d.pl.repository.owner.type === "Organization" ||
          d.pl.action === "created"
        ) {
        ...
        switch (d.pl.action) {
          ...
          case "transferred": {
            ...
            embed.title = `Repository transferred from @${
              lastOwner.organization?.login ?? lastOwner.user?.login
            } to @${d.pl.repository.owner.login}`;
            break;
          }
          default:
            break;
      ...
      case "star":
        ...
        embed.description = `${d.pl.repository.name} now has ${
          d.pl.repository.stargazers_count
        } star${d.pl.repository.stargazers_count === 1 ? "" : "s"}`;
        if (d.pl.action === "created") {
      ...
      case "status":
        ...
        embed.description = d.pl.description?.slice(0, 2048);
        embed.title = `Status updated for commit ${shortCodeCommit(
          d.pl.commit.sha,
        )}: ${d.pl.state}`;
        switch (d.pl.state) {
          case "error":
